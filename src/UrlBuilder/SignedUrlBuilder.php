...
class SignedUrlBuilder extends UrlBuilder
{
    ...
    private ?int $expiresAtTimestamp = null; // Store absolute timestamp when using expiresAt()
    ...
    public function expiresIn(int $seconds): self
    {
        ...
        $this->expiresAtTimestamp = null; // Clear absolute timestamp
        return $this;
    }
    ...
    {
        $this->expiresAtTimestamp = $timestamp; // Store absolute timestamp
        $this->expiresIn = $timestamp - time(); // Keep for backward compatibility
        return $this;
    }
    ...
    public function build(): string
    {
        ...
        // Use stored absolute timestamp if available (from expiresAt()),
        // otherwise calculate from expiresIn
        if ($this->expiresAtTimestamp !== null) {
            $this->params['exp'] = $this->expiresAtTimestamp;
        } else {
